...
def hook_arc4random(uc, address, size, user_data):
    ...
    retValue = emu.set_retval(0);
    retValue = emu.get_retval();
    print(f"hook_arc4random: {retValue}" , retValue)
    #return 0;
...
def hook_strlen(uc, address, size, user_data):
    ...
    retval = len(strlenStr)

    emu.set_retval(retval)
    print( f"strlen: {retval}")

    
def hook_caculateSome(uc, address, size, user_data):
    print("hook_caculateSome")
    emu = user_data["emu"]
    retval = emu.get_retval()
    print(retval)
    print(f"retval: {retval}")
...
def hook_caculateSomeRet(uc, address, size, user_data):
    print("hook_caculateSomeRet")
    emu = user_data["emu"]
    retval = emu.get_retval()
    print(retval)
    print(f"retval: {retval}")
    print("hook_caculateSomeRet_end")
...
def main():
    binary_path = "examples/binaries/ios/com.ttt.chomperTest2/chomperTest2"
    ...
    # ret  
    emu.add_hook(image.base + 0x0412c, hook_caculateSomeRet)

    emu.add_hook("_strlen", hook_strlen)
    #emu.add_interceptor("_strlen", hook_strlen)
    #emu.add_hook("_caculateSome", hook_caculateSome)
    ...
    #emu.add_interceptor("_caculateSome", hook_caculateSome)
...
# }
    ...
    with objc.autorelease_pool():
        ...
        #emu.call_address(image.base + 0x075198, 0,structBuf1Ptr,deliverOptions)  #  FMDIStruct_Init

        emu.call_address(image.base + 0x4054)  #  FMDIStruct_Init

        print("finished caculateSome")
