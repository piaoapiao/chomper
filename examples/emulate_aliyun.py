...
def load_offsets_from_json(file_path):
    ...
    try:
        with open(file_path, "r") as f:
            offsets = json.load(f)
        print(f"Successfully loaded offsets from {file_path}")
        return offsets
    except Exception as e:
        print(f"Failed to load offsets from {file_path}: {str(e)}")
        return []
...
def save_dict_to_json(data, file_path):
    ...
    try:
        ...
        print(f"Failed to save results to {file_path}: {str(e)}")

def main():
    binary_path = "examples/binaries/ios/com.kjjk.OCDemo/OCDemo"
    ...
    # Call function
    decrypt_result = emu.call_address(image.base + 0x9F870) # network unknown error
    decrypt_result_str = emu.read_string(decrypt_result)
    logger.info("Decrypt result: %s", decrypt_result_str)


    # decrypt_result_str = emu.call_address(image.base + 0x81348)  # </Code> 0x81348
    # decrypt_result_str = emu.read_string(decrypt_result)
    # logger.info("Decrypt result: %s", decrypt_result_str)

    # JSON 文件路径
    json_file = "caller_offsets.json"

    # 从 JSON 文件中加载偏移量数组
    all_offsets = load_offsets_from_json(json_file)
    ...
    for offset in all_offsets:
        try:
            # 调用 emu.call_address
            decrypt_result = emu.call_address(image.base + offset)
            # 读取解密后的字符串
            decrypt_result_str = emu.read_string(decrypt_result)

             # 将结果保存到字典中
            result_dict[offset] = decrypt_result_str

            # 打印解密结果
            logger.info(f"Decrypt result at offset 0x{offset:X}: {decrypt_result_str}")
        except Exception as e:
            # 如果调用失败，打印错误信息
            logger.error(f"Failed to decrypt at offset 0x{offset:X}: {str(e)}")

    # 将结果字典保存为 JSON 文件
    save_dict_to_json(result_dict, "decrypt_results.json")


    #resultStr = emu.read_string(emu.read_pointer(a5))
